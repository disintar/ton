...
void generate_py_output_to(std::ostream& os, int options = 0) {
  ...
  if (!generate_py_prepared) {
    ...
    const_pytype_expr_simple.clear();
    const_type_expr_py_idents.clear();
...
void PyTypeCode::ConsRecord::declare_record(std::ostream& os, std::string nl, int options) {
  ...
  if (n) {
    ...
    for (const ConsField& fi : py_fields) {
      ...
      if (fi.field.type->tp == TypeExpr::te_CondType) {
        os << nl << "\n            self.conditional_fields.append(\"" << fi.name << "\")";
      }
    ...
    if (pexpr->tp == TypeExpr::te_Param) {
      if (!type_param_is_neg.at(j)) {
        ...
        if (field_var_set.at(i)) {
          ...
          actions += PyAction{"self." + type_param_name.at(j) + " == " + field_vars.at(i)};
...
void PyTypeCode::add_compute_actions(const TypeExpr* expr, int i, std::string bind_to) {
  ...
  switch (expr->tp) {
    case TypeExpr::te_MulConst: {
      ...
      std::ostringstream ss;
      ss << "assert self.mul_r1(\"" << tmp << "\", " << expr->value << ", " << bind_to << "), 'Mul_r1 failed'\n";
    ...
    case TypeExpr::te_Param:
      ...
      if (!field_var_set.at(i)) {
        ...
        ss2 << "assert self." << field_vars.at(i) << " >= 0, 'Param is leq then zero'\n";
...
bool PyTypeCode::add_constraint_check(const Constructor& constr, const Field& field, int options) {
  ...
  if (expr->tp == TypeExpr::te_Apply &&
    ...
    if (x->negated || y->negated) {
      ...
      output_cpp_expr(name2, y);
      ss << name2.str();
      ss << ", 'Params not equal: " << name1.str() << " and " << name2.str() << "'\n";
      actions += PyAction{std::move(ss), true};
    }
    return true;
  } else {
  ...
  for (const TypeExpr* pexpr : constr.params) {
    ...
    if (!param_constraint_used.at(j) && !type_param_is_neg.at(j)) {
      ...
      if (!field_var_set.at(i) && pexpr->tp == TypeExpr::te_Param && pexpr->value == i) {
        ...
        if (field.type->is_nat_subtype) {
          ss2 << "assert self." << field_vars[i] << " >= 0, 'Field is leq than zero'\n";
...
void PyTypeCode::generate_unpack_method(std::ostream& os, PyTypeCode::ConsRecord& rec, int options) {
  ...
  if (options & 16) {
    ...
    os << ") and cs.empty_ext()\n\n";
    os << "            except (RuntimeError, KeyError, ValueError, AssertionError, IndexError):\n                "
          "return False\n            "
          "return True\n";
    return;
  }
  ...
  output_actions(os, nl, options | 4);
  clear_context();
...
void PyTypeCode::add_store_subrecord(std::string field_name, const ConsRecord* subrec) {
  ...
  needs_tmp_cell = true;
  std::ostringstream ss;
  ss << "cb.store_ref_or_tlb(self." << field_name << ")";
  actions += PyAction{std::move(ss)};
}
...
void PyTypeCode::output_store_field(std::ostream& os, std::string field_var, const TypeExpr* expr, py_val_type cvt) {
  ...
  switch (cvt) {
    case py_slice:
      os << "cb.store_slice_or_tlb_ext(self." << field_var << ", ";
      output_cpp_sizeof_expr(os, expr, 0);
      os << ")";
      return;
    case py_bits:
    case py_bitstring:
      assert(!(sz.max_size() & 0xff));
      os << "cb.store_bitstring_chk(self." << field_var << ", ";
      output_cpp_sizeof_expr(os, expr, 0);
      os << ")";
      return;
    case py_cell:
      ...
      if (expr->is_constexpr > 0) {
        os << "TLBComplex.constants[\"";
        output_cpp_expr(os, expr, 100);
        os << "\"].store_ref_or_tlb(cb, self." << field_var << ")";
      } else {
        os << "cb.store_ref_or_tlb(self." << field_var << ")";
      }
      return;
    case py_bool:
      assert(i > 0 && l == 1);
...
void PyTypeCode::generate_pack_field(const PyTypeCode::ConsField& fi, const Constructor& constr, const Field& field,
  ...
  if (expr->negated) {
    ...
    std::ostringstream ss;
    assert(cvt == py_slice);
    ss << "cb.store_slice_or_tlb(self." << field_vars.at(i) << ")";

    // This doesn't need because all values must be defined directly in record on pack
    std::stringstream hole;
    output_negative_type_arguments(hole, expr);
  ...
  if (!validating || (expr->args[0]->tp == TypeExpr::te_Apply &&
    ...
    ss1 << "cb.store_ref_or_tlb(self." << field_vars.at(i) << ")" << tail;
    field_var_set[i] = true;
    actions += PyAction{std::move(ss1)};
    return;
  }
  ...
  ss1 << "(cb.store_ref_or_tlb(self." << field_vars.at(i) << ") and ";
  if (!is_self(expr, constr)) {
    output_cpp_expr(ss1, expr, 100);
    ss1 << '.';
  }
  ss1 << "validate_ref(ops, " << field_vars.at(i) << "))" << tail;
  actions += PyAction{ss1.str()};
}

bool PyTypeCode::ConsRecord::declare_record_pack(std::ostream& os, std::string nl, int options) {
  bool is_ok = false;
  bool cell = options & 16;
  std::string builder_arg = cell ? "self" : "self, cb: CellBuilder";
  ...
  if (!(options & 8)) {
    os << nl << "def " << fun_name << "(" << builder_arg;
    is_ok = true;
  } else if (is_small) {
    os << nl << "def " << fun_name << "_" << cpp_type.cons_enum_name.at(cons_idx) << "(" << builder_arg;
    is_ok = true;
  }
  if (is_ok) {
    //    if (options & 2) {
...
void PyTypeCode::generate_store_enum_method(std::ostream& os, int options) {
  int minl = type.size.convert_min_size(), maxl = type.size.convert_max_size();
  bool exact = type.cons_all_exact();
  std::string ctag = "value";  // incremental_cons_tags ? "value" : "self.cons_tag[value]";
  os << "\n    def store_enum_from(self, cb: CellBuilder, value: int = None) -> bool:\n";
  if (!cons_num) {
    os << "        return False\n";
  } else if (!maxl) {
    os << "        return not bool(value)\n";
  } else if (cons_num == 1) {
    const Constructor& constr = *type.constructors.at(0);
    HexConstWriter w{constr.tag >> (64 - constr.tag_bits)};
    std::ostringstream s;
    ...
    if (exact) {
      ...
      os << "        assert value is not None and value in self.cons_tag, f'Value {value} must be in "
         << "self.cons_tag'\n"
    ...
    os << "        assert value is not None and value in self.cons_tag, f'Value {value} must be in"
       << "self.cons_tag'\n"
       << "        cb.store_uint(" << ctag << ", self.cons_len[self.cons_tag.index(" << ctag << ")])\n"
       << "        return True\n";
  }
  os << "\n";
}
