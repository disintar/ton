...
std::vector<src_func::SrcLocation> scope_opened_at;
...
void close_scope(src_func::Lexer& lex) {
  if (!scope_level) {
    throw src_func::Fatal{"cannot close the outer scope"};
  }
...
SymDef* define_global_symbol(sym_idx_t name_idx, bool force_new, const src_func::SrcLocation& loc) {
  if (!name_idx) {
    return nullptr;
  }
...
SymDef* define_symbol(sym_idx_t name_idx, bool force_new, const src_func::SrcLocation& loc) {
  if (!name_idx) {
    return nullptr;
  }
  if (!scope_level) {
