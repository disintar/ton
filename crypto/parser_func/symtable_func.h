...
struct SymDef {
  ...
  sym_idx_t sym_idx;
  SymValBase* value;
  src_func::SrcLocation loc;
  SymDef(int lvl, sym_idx_t idx, const src_func::SrcLocation& _loc = {}, SymValBase* val = 0)
...
extern std::vector<src_func::SrcLocation> scope_opened_at;

void open_scope(src_func::Lexer& lex);
void close_scope(src_func::Lexer& lex);
SymDef* lookup_symbol(sym_idx_t idx, int flags = 3);
SymDef* lookup_symbol(std::string name, int flags = 3);
void clear_sym_def();

SymDef* define_global_symbol(sym_idx_t name_idx, bool force_new = false, const src_func::SrcLocation& loc = {});
SymDef* define_symbol(sym_idx_t name_idx, bool force_new = false, const src_func::SrcLocation& loc = {});
