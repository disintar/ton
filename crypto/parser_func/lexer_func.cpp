...
std::string Lexem::lexem_name_str(int idx) {
  if (idx == Eof) {
    ...
    return "special";
  } else if (sym_func::symbols.get_keyword(idx)) {
    return "`" + sym_func::symbols.get_keyword(idx)->str + "`";
  } else {
...
std::string Lexem::name_str() const {
  if (tp == Ident) {
    return std::string{"identifier `"} + sym_func::symbols.get_name(val) + "`";
  } else if (tp == String) {
...
int Lexem::classify() {
  ...
  sym_func::sym_idx_t i = sym_func::symbols.lookup(str);
  if (i) {
    assert(str == sym_func::symbols[i]->str);
    str = sym_func::symbols[i]->str;
    sym_func::sym_idx_t idx = sym_func::symbols[i]->idx;
    tp = (idx < 0 ? -idx : Ident);
    val = i;
  ...
  if (tp == Unknown) {
    tp = Ident;
    val = sym_func::symbols.lookup(str, 1);
  }
  return tp;
}
