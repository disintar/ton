...
elseif (ANDROID OR NOT NIX)
  ...
  if (APPLE)
    ...
    if (SODIUM_CFLAGS)
      set(SODIUM_CFLAGS "${SODIUM_CFLAGS} -isysroot ${MACOS_SDK_PATH}")
    else()
      set(SODIUM_CFLAGS "-isysroot ${MACOS_SDK_PATH}")
    endif()
    if (SODIUM_CXXFLAGS)
      set(SODIUM_CXXFLAGS "${SODIUM_CXXFLAGS} -isysroot ${MACOS_SDK_PATH}")
    else()
      set(SODIUM_CXXFLAGS "-isysroot ${MACOS_SDK_PATH}")
    endif()
  endif()

  if (CMAKE_C_FLAGS)
    if (SODIUM_CFLAGS)
      set(SODIUM_CFLAGS "${SODIUM_CFLAGS} ${CMAKE_C_FLAGS} -fPIC")
    else()
      set(SODIUM_CFLAGS "${CMAKE_C_FLAGS} -fPIC")
    endif()
  else()
    if (NOT SODIUM_CFLAGS)
      set(SODIUM_CFLAGS "-fPIC")
    else()
      set(SODIUM_CFLAGS "${SODIUM_CFLAGS} -fPIC")
    endif()
  endif()
  if (CMAKE_CXX_FLAGS)
    if (SODIUM_CXXFLAGS)
      set(SODIUM_CXXFLAGS "${SODIUM_CXXFLAGS} ${CMAKE_CXX_FLAGS} -fPIC")
    else()
      set(SODIUM_CXXFLAGS "${CMAKE_CXX_FLAGS} -fPIC")
    endif()
  else()
    if (NOT SODIUM_CXXFLAGS)
      set(SODIUM_CXXFLAGS "-fPIC")
    else()
      set(SODIUM_CXXFLAGS "${SODIUM_CXXFLAGS} -fPIC")
    endif()
  endif()
