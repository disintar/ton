    ...
    namespace validator {
        ...
        public:
            PackageReader(std::shared_ptr<Package> package, td::uint64 offset,
                          ...
                          std::shared_ptr<PackageStatistics> statistics, bool read_only = false)
                    : package_(std::move(package)), offset_(offset), read_only_(read_only),
                      promise_(std::move(promise)), statistics_(std::move(statistics)) {
            }

            PackageReader(std::shared_ptr<Package> package, td::uint64 offset,
                          td::Promise<std::pair<std::string, td::BufferSlice>> promise,
                          td::Promise<td::Promise<std::pair<std::string, td::BufferSlice>>> on_fail,
                          std::shared_ptr<PackageStatistics> statistics, bool read_only = false)
                    : package_(std::move(package)), read_only_(read_only), offset_(offset),
                      on_fail_(std::move(on_fail)), promise_(std::move(promise)), statistics_(std::move(statistics)) {
                raise_on_fail = true;
            }
            ...
            void start_up() override {
                ...
                if (statistics_ && result.is_ok()) {
                    statistics_->record_read((td::Timestamp::now().at() - start.at()) * 1e6,
                                             result.ok_ref().second.size());
                }
                if (read_only_) {
        ...
        void ArchiveSlice::add_handle(BlockHandle handle, td::Promise<td::Unit> promise) {
            ...
            if (add_shard) {
                ...
                auto shard_value =
                        create_serialize_tl_object<ton_api::db_lt_shard_value>(handle->id().id.workchain,
                                                                               handle->id().id.shard);
                kv_->set(status_key.as_slice(),
                         create_serialize_tl_object<ton_api::db_lt_status_value>(idx + 1)).ensure();
        ...
        void ArchiveSlice::add_file(BlockHandle handle, FileReference ref_id, td::BufferSlice data,
            ...
            TRY_RESULT_PROMISE(
                    promise, p,
                    choose_package(
                            handle ? handle->id().is_masterchain() ? handle->id().seqno()
                                                                   : handle->masterchain_ref_block() : 0, true));
            std::string value;
            ...
            auto P = td::PromiseCreator::lambda(
                    [SelfId = actor_id(this), idx = p->idx, ref_id, promise = std::move(promise)](
                        ...
                        auto v = R.move_as_ok();
                        td::actor::send_closure(SelfId, &ArchiveSlice::add_file_cont, idx, std::move(ref_id), v.first,
                                                v.second,
                                                std::move(promise));
                    });

            td::actor::send_closure(p->writer, &PackageWriter::append, ref_id.filename(), std::move(data),
                                    std::move(P));
        }
        ...
        void
        ArchiveSlice::get_file(ConstBlockHandle handle, FileReference ref_id, td::Promise<td::BufferSlice> promise) {
            if (destroyed_) {
                promise.set_error(td::Status::Error(ErrorCode::notready, "package already gc'd"));
                return;
            }
            ...
            TRY_RESULT_PROMISE(
                    ...
                    choose_package(
                            handle ? handle->id().is_masterchain() ? handle->id().seqno()
                                                                   : handle->masterchain_ref_block() : 0, false));
            promise = begin_async_query(std::move(promise));
            auto P = td::PromiseCreator::lambda(
            ...
            if (!read_only_) {
                td::actor::create_actor<PackageReader>("reader", p->package, offset, std::move(P),
                                                       statistics_.pack_statistics, read_only_).release();
            } else {
                auto on_fail =
                        td::PromiseCreator::lambda([SelfId = actor_id(this), handle, ref_id](
                            ...
                            if (R.is_error()) {
                                ...
                                td::actor::send_closure(SelfId, &ArchiveSlice::try_get_file, handle, ref_id,
                                                        R.move_as_ok());
                            }
                        });

                td::actor::create_actor<PackageReader>("reader", p->package, offset, std::move(P), std::move(on_fail),
                                                       statistics_.pack_statistics, read_only_)
                        .release();
            }
        }
        ...
        void ArchiveSlice::try_get_file(ConstBlockHandle handle, FileReference ref_id,
            ...
            td::actor::create_actor<PackageReader>("reader", p->package, offset, std::move(promise),
                                                   statistics_.pack_statistics).release();
        }
        ...
        void ArchiveSlice::get_block_by_lt(AccountIdPrefixFull account_id, LogicalTime lt,
            ...
            return get_block_common(
                    ...
                    [lt](ton_api::db_lt_desc_value &w) {
                        return lt > static_cast<LogicalTime>(w.last_lt_) ? 1 : lt ==
                                                                               static_cast<LogicalTime>(w.last_lt_) ? 0
                                                                                                                    : -1;
                    },
                    [lt](ton_api::db_lt_el_value &w) {
                        return lt > static_cast<LogicalTime>(w.lt_) ? 1 : lt == static_cast<LogicalTime>(w.lt_) ? 0
                                                                                                                : -1;
                    },
            ...
            return get_block_common(
                    ...
                    [seqno](ton_api::db_lt_desc_value &w) {
                        return seqno > static_cast<BlockSeqno>(w.last_seqno_) ? 1
                                                                              : seqno ==
                                                                                static_cast<BlockSeqno>(w.last_seqno_)
                                                                                ? 0
                                                                                : -1;
                    },
                    [seqno](ton_api::db_lt_el_value &w) {
                        return seqno > static_cast<BlockSeqno>(w.id_->seqno_) ? 1
                                                                              : seqno ==
                                                                                static_cast<BlockSeqno>(w.id_->seqno_)
                                                                                ? 0
                                                                                : -1;
                    },
        ...
        void ArchiveSlice::get_block_by_unix_time(AccountIdPrefixFull account_id, UnixTime ts,
            ...
            return get_block_common(
                    ...
                    [ts](ton_api::db_lt_desc_value &w) {
                        return ts > static_cast<UnixTime>(w.last_ts_) ? 1 : ts == static_cast<UnixTime>(w.last_ts_) ? 0
                                                                                                                    : -1;
                    },
        ...
        void ArchiveSlice::before_query() {
            if (status_ == st_closed) {
                ...
                kv_ = std::make_unique<td::RocksDb>(td::RocksDb::open(db_path_, std::move(db_options), read_only_).move_as_ok());
                reinit();
            }
            status_ = st_open;
        ...
        ArchiveSlice::ArchiveSlice(td::uint32 archive_id, bool key_blocks_only, bool temp, bool finalized,
                                   std::string db_root,
                           td::actor::ActorId<ArchiveLru> archive_lru, DbStatistics statistics, bool read_only)
                : archive_id_(archive_id), key_blocks_only_(key_blocks_only), temp_(temp), finalized_(finalized),
                  p_id_(archive_id_, key_blocks_only_, temp_), read_only_(read_only), db_root_(std::move(db_root)),
                  archive_lru_(std::move(archive_lru)), statistics_(statistics) {
        ...
        namespace {
            ...
            void destroy_db(std::string name, td::uint32 attempt, td::Promise<td::Unit> promise) {
                ...
                delay_action(
                        [name, attempt, promise = std::move(promise)]() mutable {
                            destroy_db(name, attempt, std::move(promise));
                        },
                        td::Timestamp::in(1.0));
            }
        }  // namespace
        ...
        void ArchiveSlice::destroy(td::Promise<td::Unit> promise) {
            ...
            delay_action([name = db_path, attempt = 0,
                                 promise = ig.get_promise()]() mutable {
                             destroy_db(name, attempt, std::move(promise));
                         },
                         td::Timestamp::in(0.0));
        }
        ...
        void
        ArchiveSlice::truncate(BlockSeqno masterchain_seqno, ConstBlockHandle handle, td::Promise<td::Unit> promise) {
            ...
            for (int i = 0; i < f->total_shards_; i++) {
                ...
                truncate_shard(masterchain_seqno, ShardIdFull{g->workchain_, static_cast<td::uint64>(g->shard_)},
                               pack->idx,
                               new_package.get());
            }
