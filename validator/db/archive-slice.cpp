...
namespace ton {
    ...
    namespace validator {
          ...
          TRY_RESULT_PROMISE(
                  ...
                  choose_package(
                          handle ? handle->id().is_masterchain() ? handle->id().seqno()
                                                                 : handle->masterchain_ref_block() : 0,
        ...
        void ArchiveSlice::before_query() {
          if (status_ == st_closed) {
            ...
            kv_ = std::make_unique<td::RocksDb>(
                    td::RocksDb::open(db_path_, std::move(db_options), read_only_).move_as_ok());
            reinit();
          }
          status_ = st_open;
        ...
        ArchiveSlice::ArchiveSlice(td::uint32 archive_id, bool key_blocks_only, bool temp, bool finalized, td::uint32 shard_split_depth,
                                   std::string db_root,
            ...
            void destroy_db(std::string name, td::uint32 attempt, td::Promise<td::Unit> promise) {
              ...
              delay_action(
                      [name, attempt, promise = std::move(promise)]() mutable {
                          destroy_db(name, attempt + 1, std::move(promise));
                      },
                      td::Timestamp::in(1.0));
            }
        }  // namespace
