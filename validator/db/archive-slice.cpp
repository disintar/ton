...
void ArchiveSlice::get_temp_handle(BlockIdExt block_id, td::Promise<ConstBlockHandle> promise) {
  ...
  promise.set_value(std::move(handle));
    ...
    ArchiveSlice::get_file(ConstBlockHandle handle, FileReference ref_id, td::Promise<td::BufferSlice> promise) {
      ...
      R.ensure();
      if (R.move_as_ok() == td::KeyValue::GetStatus::NotFound) {
        promise.set_error(td::Status::Error(ErrorCode::notready, PSTRING() << "file " << ref_id.filename() << " not in archive slice " << get_name()));
        return;
      }
      ...
      if (!read_only_) {
        td::actor::create_actor<PackageReader>(PSTRING() << "reader." << td::PathView(p->path).file_name(), p->package, offset, std::move(P),
        ...
        td::actor::create_actor<PackageReader>(PSTRING() << "reader." << td::PathView(p->path).file_name(), p->package, offset, std::move(P), std::move(on_fail),
    ...
    void ArchiveSlice::try_get_file(ConstBlockHandle handle, FileReference ref_id,
      ...
      td::actor::create_actor<PackageReader>(PSTRING() << "reader." << td::PathView(p->path).file_name(), p->package, offset, std::move(promise),
