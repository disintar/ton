...
void LiteQuery::abort_query(td::Status reason) {
  auto query_id = query_obj_ ? query_obj_->get_id() : 0;
  if (dst_.is_zero()) {
    td::actor::send_closure(manager_, &ValidatorManager::add_lite_query_stats, query_id, false);
  } else {
    td::actor::send_closure(manager_, &ValidatorManager::add_lite_query_stats_extended, query_id, dst_, started_at_,
                            std::time(nullptr), false);
  }
  LOG(INFO) << "aborted liteserver query: " << reason.to_string()
            << " query=" << (query_obj_ ? lite_query_name_by_id(query_id) : "unknown")
            << " compiled=" << compiled_query_string;
...
bool LiteQuery::finish_query(td::BufferSlice result, bool skip_cache_update) {
  ...
  if (promise_) {
    auto query_id = query_obj_ ? query_obj_->get_id() : 0;
    if (dst_.is_zero()) {
      td::actor::send_closure(manager_, &ValidatorManager::add_lite_query_stats, query_id, true);
    } else {
      td::actor::send_closure(manager_, &ValidatorManager::add_lite_query_stats_extended, query_id, dst_, started_at_,
                              std::time(nullptr), true);
    }
...
void LiteQuery::perform() {
  if (query_obj_) {
    compiled_query_string = PSTRING() << "Query: " << lite_query_name_by_id(query_obj_->get_id());
  }
