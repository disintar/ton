...
inline td::Result<td::BufferSlice> serialize_block_broadcast(const BlockBroadcast& broadcast, const char* called_from) {
  return serialize_block_broadcast(broadcast, std::string(called_from));
}
