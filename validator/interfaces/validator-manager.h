...
class ValidatorManager : public ValidatorManagerInterface {
 public:
  ...
  virtual void get_ihr_messages(ShardIdFull shard, td::Promise<std::vector<td::Ref<IhrMessage>>> promise) {
    promise.set_value({});
  }
  ...
  virtual void send_get_block_request(BlockIdExt id, td::uint32 priority, td::Promise<ReceivedBlock> promise) {
  ...
  virtual td::actor::Task<ReceivedBlock> send_get_block_request(BlockIdExt id, td::uint32 priority) {
    ...
    send_get_block_request(id, priority, std::move(promise));
  ...
  virtual void send_ihr_message(td::Ref<IhrMessage> message) {
  }
  ...
  virtual void update_lite_server_state(BlockIdExt shard_client, td::Ref<MasterchainState> state) {
  }
