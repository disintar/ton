...
void ValidatorManagerImpl::validate_block_broadcast_signatures(BlockBroadcast broadcast,
  ...
  td::actor::create_actor<ValidateBroadcast>("broadcast-sigcheck", std::move(broadcast), last_masterchain_block_handle_,
                                             last_masterchain_state_, last_known_key_block_handle_, publisher_.get(), actor_id(this),
...
constexpr double LIVE_ARCHIVE_RESYNC_LAG_SEC = 12.0;
constexpr double LIVE_ARCHIVE_RESYNC_CHECK_SEC = 2.0;
constexpr double LIVE_ARCHIVE_RESYNC_DONE_COOLDOWN_SEC = 3.0;
...
bool ValidatorManagerImpl::out_of_sync() {
  ...
  if (shard_client_handle_->unix_time() + 8 <= now) {
    return true;
  }
  if (last_masterchain_block_handle_->unix_time() + 8 > now) {
    return false;
  }
...
bool ValidatorManagerImpl::live_archive_resync_needed() const {
  if (!last_masterchain_block_handle_ || !shard_client_handle_) {
    return false;
  }
  auto now = td::Clocks::system();
  auto master_lag = now - last_masterchain_block_handle_->unix_time();
  auto shard_client_lag = now - shard_client_handle_->unix_time();
  if (master_lag > LIVE_ARCHIVE_RESYNC_LAG_SEC || shard_client_lag > LIVE_ARCHIVE_RESYNC_LAG_SEC) {
    return true;
  }
  if (shard_client_handle_->id().seqno() + 16 < last_masterchain_seqno_) {
    return true;
...
void ValidatorManagerImpl::maybe_start_live_archive_resync() {
  if (!prestart_sync_completed_ || !started_ || archive_sync_running_ || !live_archive_resync_at_.is_in_past()) {
    return;
  }
  live_archive_resync_at_ = td::Timestamp::in(LIVE_ARCHIVE_RESYNC_CHECK_SEC);
  alarm_timestamp().relax(live_archive_resync_at_);
  if (!live_archive_resync_needed()) {
    return;
  }

  auto now = td::Clocks::system();
  LOG(WARNING) << "[archive-sync] stage=live_resync.start"
               << " master_lag=" << (now - last_masterchain_block_handle_->unix_time())
               << " shard_lag=" << (now - shard_client_handle_->unix_time())
               << " master_seqno=" << last_masterchain_seqno_
               << " shard_client_seqno=" << shard_client_handle_->id().seqno() << " result=start";
  archive_sync_running_ = true;
...
void ValidatorManagerImpl::download_next_archive() {
  if (!out_of_sync() && !(prestart_sync_completed_ && live_archive_resync_needed())) {
...
void ValidatorManagerImpl::finish_prestart_sync() {
  to_import_.clear();
  archive_sync_running_ = false;
  if (prestart_sync_completed_) {
    auto now = td::Clocks::system();
    LOG(WARNING) << "[archive-sync] stage=live_resync.done"
                 << " master_lag=" << (now - last_masterchain_block_handle_->unix_time())
                 << " shard_lag=" << (now - shard_client_handle_->unix_time())
                 << " master_seqno=" << last_masterchain_seqno_
                 << " shard_client_seqno=" << shard_client_handle_->id().seqno() << " result=ok";
    live_archive_resync_at_ = td::Timestamp::in(LIVE_ARCHIVE_RESYNC_DONE_COOLDOWN_SEC);
    alarm_timestamp().relax(live_archive_resync_at_);
    callback_->archive_sync_complete(last_masterchain_block_handle_);
    return;
  }
  completed_prestart_sync();
}
...
void ValidatorManagerImpl::update_shards() {
  ...
  auto P2 =
          td::PromiseCreator::lambda([SelfId = actor_id(this),
...
void ValidatorManagerImpl::alarm() {
  ...
  maybe_start_live_archive_resync();
