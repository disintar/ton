...
class ManagerFacadeImpl : public ManagerFacade {
 public:
  ...
  void cache_block_candidate(BlockCandidate candidate) override {
    auto new_block_candidate_broadcast = static_cast<td::actor::Task<> (ValidatorManager::*)(
        BlockIdExt, CatchainSeqno, td::BufferSlice, BroadcastSource)>(&ValidatorManager::new_block_candidate_broadcast);
    td::actor::ask(manager_, new_block_candidate_broadcast, candidate.id, validator_set_->get_catchain_seqno(),
                   candidate.data.clone(), BroadcastSource::consensus_overlay)
        .detach();
  }
