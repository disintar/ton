...
void ShardClient::start() {
  started_ = true;
  LOG(WARNING) << "[shardclient-sync] stage=start mc=" << masterchain_block_handle_->id().to_str() << " result=ok";
...
void ShardClient::get_current_shards(td::Promise<std::vector<BlockIdExt>> promise) {
  auto latest_shards = latest_shards_;
  promise.set_value(std::move(latest_shards));
}

void ShardClient::force_update_shard_client_ex(BlockHandle handle, Ref<MasterchainState> state, td::Promise<> promise) {
  [](ShardClient *self, BlockHandle handle, Ref<MasterchainState> state, td::Promise<> promise) -> td::actor::Task<> {
    auto R = co_await self->force_update_shard_client(std::move(handle), std::move(state)).start().wrap();
    promise.set_result(std::move(R));
    co_return {};
  }(this, std::move(handle), std::move(state), std::move(promise))
      .start()
      .detach();
}

td::actor::Task<> ShardClient::force_update_shard_client(BlockHandle handle, Ref<MasterchainState> state) {
  co_await initialize_waiter_.get();
  CHECK(!init_mode_);
  CHECK(masterchain_block_handle_);
