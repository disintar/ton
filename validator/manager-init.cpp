...
=======
      ...
      if (result.is_error()) {
        LOG(ERROR) << "wait state error: " << result.move_as_error();
>>>>>>> /var/folders/3k/91ytkdls3l93dl_snvs85g2r0000gn/T/tmp.oFoKjBdqyy
        continue;
      }
    }
  ...
  while (true) {
    auto r_state = co_await td::actor::ask(manager_, &ValidatorManager::get_shard_state_from_db, handle_).wrap();
    if (r_state.is_error()) {
      if (read_only_) {
        // In read-only mode, wait until the shard state is visible in cell DB.
        co_await td::actor::coro_sleep(td::Timestamp::in(0.1));
        continue;
      }
      r_state.ensure();
    }
    state_ = td::Ref<MasterchainState>{r_state.move_as_ok()};
    break;
  }
  ...
  if (read_only_) {
    co_return ValidatorManagerInitResult{handle_, state_, {}, {}, {}, {}};
  }
