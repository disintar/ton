...
static TypePtr pick_exact_type_if_generics_omitted(TypePtr expr_type, TypePtr cmp_type) {
  if (const auto* t_struct = cmp_type->try_as<TypeDataStruct>();
...
=======
>>>>>>> /var/folders/3k/91ytkdls3l93dl_snvs85g2r0000gn/T/tmp.m46NcVqeT6
/*
...
=======
        if (substitutedTs.typeT_at(i)->get_width_on_stack() != 1 &&
            !is_allowed_asm_generic_function_with_non1_width_T(fun_ref, substitutedTs, i)) {
>>>>>>> /var/folders/3k/91ytkdls3l93dl_snvs85g2r0000gn/T/tmp.m46NcVqeT6
...
=======
        if (const TypeDataAlias* r_aliasT = receiver_type->try_as<TypeDataAlias>();
            r_aliasT && r_aliasT->alias_ref->is_generic_alias()) {
          if (TypePtr hinted_receiver =
                  hint ? try_pick_instantiated_generic_from_hint(hint, r_aliasT->alias_ref) : nullptr) {
            receiver_type =
                hinted_receiver;  // assigned to `var v: Maybe<int> = Maybe.none()` or an alias-equivalent hint
>>>>>>> /var/folders/3k/91ytkdls3l93dl_snvs85g2r0000gn/T/tmp.m46NcVqeT6
          } else {
  ...
  ExprFlow infer_function_call(V<ast_function_call> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    for (int i = 0; i < v->get_num_args(); ++i) {
      LocalVarPtr param_ref =
          delta_self + i < fun_ref->get_num_params() ? &fun_ref->parameters[delta_self + i] : nullptr;
...
=======
    TypePtr inferred_type =
        fun_ref->does_return_self() ? fun_ref->parameters[0].declared_type : fun_ref->inferred_return_type;
>>>>>>> /var/folders/3k/91ytkdls3l93dl_snvs85g2r0000gn/T/tmp.m46NcVqeT6
    assign_inferred_type(v, inferred_type);
...
=======
      bool is_valid_constructor =
          provided_type->try_as<TypeDataArray>() ||
          (provided_type->try_as<TypeDataStruct>() &&
           provided_type->try_as<TypeDataStruct>()->struct_ref->is_instantiation_of_LispListT()) ||
          (provided_type->try_as<TypeDataMapKV>()) || (provided_type->try_as<TypeDataShapedTuple>());
>>>>>>> /var/folders/3k/91ytkdls3l93dl_snvs85g2r0000gn/T/tmp.m46NcVqeT6
...
=======
      ...
      if (v_arm->pattern_kind == MatchArmKind::exact_type) {
        TypePtr exact_type = pick_exact_type_if_generics_omitted(v->get_subject()->inferred_type,
                                                                 v_arm->pattern_type_node->resolved_type);
...
>>>>>>> /var/folders/3k/91ytkdls3l93dl_snvs85g2r0000gn/T/tmp.m46NcVqeT6
  ...
  ExprFlow infer_lambda_fun(V<ast_lambda_fun> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    for (LocalVarPtr captured_var_ref : v->captured_vars) {
      TypePtr captured_type =
          flow.smart_cast_or_original(SinkExpression(captured_var_ref), captured_var_ref->declared_type);
