      ...
      */
          ...
          if (info.after_merge) {
            ...
            if (info.not_master && !is_first) {
              ...
              auto P1 = td::PromiseCreator::lambda([this, workchain_shard = blkid.id.workchain,
                                                    seqno_shard = prev_blk_1.seq_no, shard_shard = blkid.id.shard,
                                                    SelfId = SelfId, first = is_first](td::Result<ConstBlockHandle> R) {
                if (R.is_error()) {
                  LOG(ERROR) << "ERROR IN BLOCK: "
                             << "Seqno: " << seqno_shard << " Shard: " << shard_shard
              ...
              auto P2 = td::PromiseCreator::lambda([this, workchain_shard = blkid.id.workchain,
                                                    seqno_shard = prev_blk_2.seq_no, shard_shard = blkid.id.shard,
                                                    SelfId = SelfId, first = is_first](td::Result<ConstBlockHandle> R) {
                if (R.is_error()) {
                  LOG(ERROR) << "ERROR IN BLOCK: "
                             << "Seqno: " << seqno_shard << " Shard: " << shard_shard
              ...
              const auto id = std::to_string(blkid.id.workchain) + ":" + std::to_string(blkid.id.shard) + ":" +
                              std::to_string(prev_blk_1.seq_no);
              LOG(DEBUG) << "Receive start_parse_shards for " << id;
              ...
              if (already_traversed_.find(id) != already_traversed_.end()) {
                ...
                td::actor::send_closure(validator_manager_, &ValidatorManagerInterface::get_block_by_seqno_from_db, pfx,
                                        prev_blk_1.seq_no, std::move(P1));

                td::actor::send_closure(validator_manager_, &ValidatorManagerInterface::get_block_by_seqno_from_db, pfx,
                                        prev_blk_2.seq_no, std::move(P2));
              } else {
                LOG(DEBUG) << id << " <- already traversed!";
              }
//
       ...
       */
          ...
          if ((int)extra.custom->prefetch_ulong(1) == 1) {
            ...
            auto parseShards = [this, &shards_json, SelfId, &blkid, is_first](McShardHash &ms) {
              ...
              auto P = td::PromiseCreator::lambda([this, workchain_shard = shard_workchain, seqno_shard = shard_seqno,
                                                   shard_shard = shard_shard, SelfId = SelfId,
                                                   first = is_first](td::Result<ConstBlockHandle> R) {
                if (R.is_error()) {
                  LOG(ERROR) << "ERROR IN BLOCK: "
                             << "Seqno: " << seqno_shard << " Shard: " << shard_shard
                             << " Worckchain: " << workchain_shard;
              ...
              const auto id = std::to_string(shard_workchain) + ":" + std::to_string(shard_shard) + ":" +
        ...
        td::PromiseCreator::lambda([this, SelfId = actor_id(this), block_id_string = id, handle,
          ...
          if (R.is_error()) {
            ...
            try {
              ...
              json total_balance = {{"grams", block::tlb::t_Grams.as_integer(total_balance_cc.grams)->to_dec_string()},
                                    {"extra", total_balance_cc.other->have_refs()
                                                  ? parse_extra_currency(total_balance_cc.other->prefetch_ref())
                                                  : dummy}};
              ...
              auto accounts = td::make_unique<vm::AugmentedDictionary>(
                  vm::load_cell_slice(shard_state.accounts).prefetch_ref(), 256, block::tlb::aug_ShardAccounts);
              ...
              if (accounts_keys.empty()) {
                ...
                for (const auto &account : accounts_keys) {
                  ...
                  if (value.not_null()) {
                    ...
                    LOG(DEBUG) << "Parse accounts states account data parsed " << account.to_hex() << " "
                               << block_id_string << " " << timer;
                    ...
                    if (acc_tag == block::gen::t_Account.account) {
                      ...
                      data["account"]["storage"]["balance"] = {
                          ...
                          {"extra",
                           balance.other->have_refs() ? parse_extra_currency(balance.other->prefetch_ref()) : dummy}};

                      LOG(DEBUG) << "Parse accounts states account storage parsed" << account.to_hex() << " "
                                 << block_id_string << " " << timer;
                  ...
                  LOG(DEBUG) << "Parse accounts states account finally parsed " << account.to_hex() << " "
                             << block_id_string << " " << timer;
                }
