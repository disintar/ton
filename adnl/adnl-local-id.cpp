...
void AdnlLocalId::publish_address_list() {
  if (addr_list_.empty() || (addr_list_.size() == 0 && !addr_list_.has_reverse())) {
    VLOG(adnl, INFO) << this << ": skipping public addr list, because localid not fully initialized";
    return;
  }
  if (dht_node_.empty()) {
    VLOG(adnl, INFO) << this << ": skipping public addr list, because DHT not fully initialized";
    return;
  }
